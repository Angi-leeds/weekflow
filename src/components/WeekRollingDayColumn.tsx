import type { CalendarItem, Category, ItemDisplayOptions, DateHeaderDisplayOptions, ListDisplayOptions, MultiDayAllDayLayout, TodayHighlightOptions } from '../types'
...
export function WeekRollingDayColumn({
  ...
  const selectDay = () => onSelectDate?.(date)
  ...
  if (mode === 'timeline') {
    ...
    return (
      <div
        ...
        } ${columnHighlight.className} ${selectionClass}`}
        style={{ width: dayWidth, ...columnHighlight.style }}
      >
        <button
          type="button"
          onClick={selectDay}
          className={`relative w-full shrink-0 border-b border-wf-border px-1 py-2 text-left ${headerHighlight.className} ${selectionClass}`}
          style={headerHighlight.style}
          aria-label={`Select ${toISODate(date)}`}
  ...
  return (
    <div
      ...
      className={`box-border flex h-full shrink-0 flex-col overflow-hidden ${
        showRightBorder ? 'border-r border-wf-border' : ''
      } ${today ? columnHighlight.className : 'bg-wf-surface'} ${selectionClass}`}
      style={{ width: dayWidth, ...(today ? columnHighlight.style : undefined) }}
    >
      <button
        type="button"
        onClick={selectDay}
        className={`relative w-full shrink-0 border-b border-wf-border px-0.5 py-2.5 text-left ${
