...
import { fetchGoogleDriveFolders } from '../lib/google'

interface SelectedGoogleDriveFolder {
  id: string
  label: string
  url?: string
}
...
interface GoogleDriveFolderPickerProps {
  ...
  onSelect: (folder: SelectedGoogleDriveFolder) => void
}
...
export function GoogleDriveFolderPicker({
  ...
  const [breadcrumbs, setBreadcrumbs] = useState<Breadcrumb[]>([{ name: 'Google Drive' }])
  ...
  const loadFolders = useCallback(async () => {
    ...
    setError(null)
    try {
      const items = await fetchGoogleDriveFolders(accountId, currentParentId)
      setFolders(items)
    } catch (loadError) {
  ...
  const folderLabel = (folder: { name: string }) => {
    const path = [...breadcrumbs.slice(1).map((entry) => entry.name), folder.name].join(' / ')
    return path ? `Google Drive / ${path}` : `Google Drive / ${folder.name}`
  }
